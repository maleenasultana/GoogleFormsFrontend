...
export const SignUp = () => {
  ...
  const signUp = async () => {
    const loginUser = { ...user, email: user.email + '@gmail.com' }
    signUpUser(loginUser).then(res => {
      if (res.data.status) {
        navigate('/signin');
      }
  ...
  return (
    <div className="signup-container">
      <div>
        ...
        <br />
        <div>
          <button className="signup-page-btn">Sign in instead</button>
          <button className="next-btn" onClick={signUp}>
            Next
          </button>
        </div>
      </div>
